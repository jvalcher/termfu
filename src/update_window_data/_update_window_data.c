               ...
               state_t *state)
{
    ...
    case Brk:
        switch (curr_debugger) {
        case (DEBUGGER_GDB):
            send_command (state, 2, "info ", "break\n");
    ...
    case LcV:
        switch (curr_debugger) {
        case (DEBUGGER_GDB):
            send_command (state, 2, "info ", "locals\n");
            break;
        }
        break;
    case Reg:
        switch (curr_debugger) {
        case (DEBUGGER_GDB):
            send_command (state, 2, "info ", "registers\n");
            break;
        }
        break;
    case Src:
        switch (curr_debugger) {
        case (DEBUGGER_GDB):
            send_command (state, 2, "info ", "line\n");
            break;
        }
        break;
    case Wat:
        switch (curr_debugger) {
        case (DEBUGGER_GDB):
            send_command (state, 2, "info ", "watchpoints\n");
            break;
        }
        break;
    }
...
display_lines_buff (int key,
                    window_t *win)
{
    char   *buff_ptr = win->buff_data->ptr,
           *newline_ptr;
    int     wy = 0,
            wx = 0,
            data_row;
...
void
get_num_file_rows_cols (window_t *win) {
